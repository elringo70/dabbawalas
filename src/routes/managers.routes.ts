...
class ManagerRoutes {
    ...
    private config() {
        ...
        this.router.get('/postNewCashierPage', [checkJWT.checkJWT, checkRole(['M'])], managerControllers.postNewCashierPage)

        //Post new cashier validations
        this.router.post('/postNewCashier', [checkJWT.checkJWT, checkRole(['M'])], [
            check('name')
                .not().isEmpty().withMessage('Ingrese su nombre')
                .trim()
                .isLength({ min: 3 }).withMessage('El nombre debe de contener al menos 3 caracteres')
            ...
            check('pass')
                ...
                .trim()
                .isStrongPassword({ minLength: 6, minUppercase: 1, minSymbols: 1 }).withMessage('Ingrese la contraseña como se le indica')
        ], managerControllers.postNewCashier)
        this.router.get('/getCashiersPage', [checkJWT.checkJWT, checkRole(['M', 'A'])], managerControllers.getCashiersPage)
        this.router.get('/getCashiers', [checkJWT.checkJWT, checkRole(['M', 'A'])], managerControllers.getCashiers)
